...
public class GLFWKeyboard {
	...
	private int tmp_key = 0;
	private boolean tmp_repeat = false;
	private int tmp_char = 0;

	public GLFWKeyboard(GLFWDisplay display) {
	...
	public void createKeyboard() {
		this.charCallback = GLFWCharCallback.create(new GLFWCharCallbackI() {
			public void invoke(long window, int codepoint) {
				...
				if (tmp_char != 0) {
					putKeyboardEvent(0, (byte)1, tmp_char, System.nanoTime(), false);
				}
				tmp_char = codepoint; // Remember current character
			}
		});
		...
		this.keyCallback = GLFWKeyCallback.create(new GLFWKeyCallbackI() {
			public void invoke(long window, int glfwKey, int scancode, int action, int mods) {
				...
				// If we haven't processed last key, put it into event queue.
				if (tmp_key != 0) {
					putKeyboardEvent(tmp_key, key_down_buffer[tmp_key], 0, System.nanoTime(), tmp_repeat);
				}
				tmp_repeat = action == GLFW.GLFW_REPEAT;
				tmp_key = key;
			}
		});
		long handle = display.getHandle();
		GLFW.glfwSetCharCallback(handle, this.charCallback);
		GLFW.glfwSetKeyCallback(handle, this.keyCallback);
	}
	...
	public void readKeyboard(ByteBuffer readBuffer) {
		if (tmp_key != 0 && tmp_char != 0) {
			// We need to remember both key and character from glfw callbacks because lwjgl 2 sends them in a single event.
			putKeyboardEvent(tmp_key, key_down_buffer[tmp_key], tmp_char, System.nanoTime(), tmp_repeat);
			tmp_key = 0;
			tmp_char = 0;
		} else if (tmp_key != 0) {
			// Character without associated key
			putKeyboardEvent(tmp_key, key_down_buffer[tmp_key], 0, System.nanoTime(), tmp_repeat);
			tmp_key = 0;
		} else if (tmp_char != 0) {
			// Key without associated character (modifier keys and function keys)
			putKeyboardEvent(0, (byte)1, tmp_char, System.nanoTime(), false);
			tmp_char = 0;
		}
